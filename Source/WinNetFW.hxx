...
namespace FWMFW
{
	namespace WinNetFW
	{
		...
		public:
			...
			std::unordered_map<std::string, std::string> getRules(const FilterFunction&& ruleNameFilter = nullptr, const FilterFunction&& appNameFilter = nullptr) const;

			typedef std::function<void(const std::string&)> RuleChangedCallback;

			// technically, these two functions does not modify the class itself (as well as the two pointer data members) so they can be marked as "const"
			// arg: map<appName, ruleName>
			std::size_t addBlockRules(const std::unordered_map<std::string, std::string>& rules, const RuleChangedCallback&& fileBlockAddedCallback = nullptr);
			std::size_t removeBlockRules(const std::unordered_map<std::string, std::string>& rules, const RuleChangedCallback&& fileBlockRemovedCallback = nullptr);
